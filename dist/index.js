...
(() => {
  ...
  var getAnimationValues = (element, defaultValues) => {
    const {
      animationType,
      delay,
      duration,
  ...
  for (let i = 0; i < charRevealElements.length; i++) {
    ...
    const charRevealParentEl = charRevealEl.closest(selectors.revealParent);
    const {
      animationType,
      delay,
      duration,
    ...
    const charElements = [...charRevealEl.querySelectorAll(".char")];
    const lineElements = [...charRevealEl.querySelectorAll(".line")];
    const targetElements = (revealType === "lines" ? lineElements : revealType === "words" ? wordElements : charElements) || [];
    const tweenProps = {
      delay,
    ...
    if (animationType === "from-bottom") {
      gsapWithCSS.set(targetElements, { yPercent: 100 });
    }
      ...
      (entries2) => {
        entries2.forEach((entry) => {
          if (entry.isIntersecting) {
            gsapWithCSS.to(targetElements, { ...tweenProps, y: 0, yPercent: 0, x: 0, opacity: 1 });
